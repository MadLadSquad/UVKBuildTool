...
if (APPLE))" << std::endl;
    ...
    stream << "    add_executable("<< name << " ${"<< name << "Src} Engine/ThirdParty/logger/UVKLog.h Engine/ThirdParty/logger/UVKLog.cpp Engine/ThirdParty/stb/stb_image.h Engine/ThirdParty/stb/sndfile.h)" << std::endl;
    stream << "else()" << std::endl;
    stream << "    add_executable("<< name << " ${"<< name << "Src} Engine/ThirdParty/logger/UVKLog.h Engine/ThirdParty/logger/UVKLog.cpp Engine/ThirdParty/stb/stb_image.h)" << std::endl;
    stream << "endif()" << std::endl;
    ...
    stream << "        target_link_libraries("<< name << " glfw opengl32 libglew_static yaml-cpp vulkan-1 assimp freetype)" << std::endl;
    stream << "    else()" << std::endl;
    stream << "        target_compile_options("<< name << " PRIVATE \"/O2bi\" \"/arch:AVX2\")" << std::endl;
    stream << "        target_link_libraries("<< name << " glfw OpenAL opengl32 libglew_static yaml-cpp vulkan-1 sndfile assimp freetype)" << std::endl;
    stream << "    endif()" << std::endl;
